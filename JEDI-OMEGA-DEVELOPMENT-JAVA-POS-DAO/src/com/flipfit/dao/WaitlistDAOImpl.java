...
public class WaitlistDAOImpl implements WaitlistDAO {
    ...
    public Waitlist getNextInLine(int slotId, Date date) {
        ...
        String sql = "SELECT * FROM waitlist WHERE slot_id = ? AND booking_date = ? ORDER BY waitlist_id ASC LIMIT 1";
    ...
    public List<Waitlist> getWaitlistByUserId(int userId) {
        ...
        String sql = "SELECT w.waitlist_id, w.user_id, w.slot_id, w.booking_date, c.name, s.start_time FROM waitlist w JOIN slot s ON w.slot_id = s.slot_id JOIN gym_center c ON s.center_id = c.center_id WHERE w.user_id = ? AND NOT EXISTS ( SELECT 1 FROM booking b WHERE b.user_id = w.user_id AND b.slot_id = w.slot_id AND b.booking_date = w.booking_date)";
